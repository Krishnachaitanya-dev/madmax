...
import { ArrowRight, Truck, ArrowLeft, Save } from "lucide-react-native";
...
export default function PickedUpOrdersScreen() {
  ...
  // Debug logging
  useEffect(() => {
    console.log("PickedUpOrdersScreen mounted");
    return () => {
      console.log("PickedUpOrdersScreen unmounted");
    };
  }, []);
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      console.log(`Fetched ${data?.length || 0} picked up orders`);
              ...
              onPress={() => {
                Alert.alert(
                  ...
                  [
                    ...
                    { text: "Update", onPress: () => updateOrderStatus(String(item.id), "processing") }
                  ]
                );
              }}
            >
  ...
  const renderEmptyList = () => (
    <View style={styles.emptyContainer}>
      <Text style={styles.emptyText}>No picked up orders found</Text>
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Picked Up Orders</Text>
